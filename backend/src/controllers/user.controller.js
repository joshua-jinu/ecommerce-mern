...
export const createUser = async (req, res) => {
  ...
  if (!name || !email || !password) {
    ...
    return res
      .status(204)
      .send({ success: false, message: "Data not present" });
  }
...
export const signup = async (req, res) => {
  ...
  try {
    ...
    bcrypt.hash(password, 10, async function (err, hash) {
      try {
        ...
        return res.status(500).send({
          success: false,
          message: `Error in User creation: ${err.message}`,
        });
      }
    });
  } catch (err) {
    ...
    return res.status(500).send({
      success: false,
      message: `Server error in signup: ${err.message}`,
    });
  }
};
...
export const deleteAddress = async (req, res) => {
  ...
  try {
    if (!mongoose.Types.ObjectId.isValid(addressId)) {
      return res
        .status(404)
        .send({ message: "Invalid Address ID", success: false });
    }
    if (!mongoose.Types.ObjectId.isValid(userId)) {
      return res
        .status(404)
    ...
    return res
      .status(201)
      .send({ message: "User address deleted", success: true });
  } catch (err) {
    console.log(err.message);
